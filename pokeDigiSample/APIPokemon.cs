...
namespace pokeDigiSample
{
    public class APIPokemon
    {
        ...
        public string creatureUrl { get; set; }
        ...
        public static List<IPokemon> MapAPIToDB(List<APIPokemon> APICreatures, List<IPokemon> DBCreatures, string creatureUrl)
        {
            foreach (APIPokemon creature in APICreatures)
            {
                IPokemon pokemon = new IPokemon();
                creature.creatureUrl = creatureUrl;
                pokemon.Url = creatureUrl;
                pokemon.Name = creature.name;
                pokemon.APIPrefix = GetAPIPrefix();
                pokemon.Image = creature.sprites.Convert().FirstOrDefault();
                pokemon.Types = GetPokemonTypes(creature.types);
                DBCreatures.Add(pokemon);
            }
            return DBCreatures;
        }

        public static IPokemon MapAPIToDB(APIPokemon creature, IPokemon pokemon, string creatureUrl)
        {
            creature.creatureUrl = creatureUrl;
            pokemon.Url = creatureUrl;
            pokemon.Name = creature.name;
        ...
        public IPokemon MapAPIToDB(string creatureUrl)
        {
            IPokemon pokemon = new IPokemon();
            ...
            pokemon.Name = name;
            pokemon.APIPrefix = GetAPIPrefix();
            pokemon.Image = sprites.Convert().FirstOrDefault();
            pokemon.Types = GetPokemonTypes(types);
            return pokemon;

        }

        public static List<string> GetPokemonTypes(Type[] types)
        {
            List<string> results = new List<string>();
            foreach (Type type in types)
            {
                results.Add(type.type.name.ToLower());
            }
            return results;
        }
    }
    ...
    public class Sprites
    {
        ...
        public PokemonSprite Convert()
        {
            PokemonSprite sprite = new PokemonSprite(back_default, back_female, back_shiny, back_shiny_female, front_default, front_female, front_shiny, front_shiny_female);
            return sprite;
        }
    }
